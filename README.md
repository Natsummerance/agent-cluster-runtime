...
（各版本资产与 Release Notes 见对应 tag，最新版为 [v0.6.4](https://github.com/Natsummerance/agent-cluster-runtime/releases/tag/v0.6.4)）：

- **Windows**：`AgentClusterWorkbench-Setup-0.6.4.exe`（x64+arm64 合并安装包；或按架构的 `Setup 0.6.4 x64.exe` / `Setup 0.6.4 arm64.exe`）
- **macOS**：`AgentClusterWorkbench-0.6.4-<arch>-unsigned.dmg` / `.zip`（x64 与 arm64；本轮未签名，首次打开需右键「打开」绕过 Gatekeeper）
- **Linux**：`AgentClusterWorkbench-0.6.4-amd64.deb`（Debian/Ubuntu x64）
