...
> 版本：0.6.1 ｜ 底座：Python 3.11+ / LangGraph / pydantic v2 ｜ 前端：React+Vite+Electron ｜ 默认零 LLM 依赖可运行
...
- **工具执行层（v0.2）**：内置工具 + 三级权限分层 + 工作区路径越界拦截 + 危险工具
...
**已实现（0.3.0）—— 会话式产品构建 + Token 制规划计量**

- `agent-cluster build --goal "..."`：一个需求驱动全生命周期（启动会 → PM 澄清 + PRD →
  需求评审/门 → 架构设计 → 设计评审/门 → 前后端/算法并行开发 → 代码评审 → QA 真实测试 →
  迭代门 → 文档 → 部署产物 + 本地冒烟 → 发布门 → 交付组装）。
- **Token 即时间**：预算（默认 500K，`--budget` 可配）→ 任务/阶段 token 预估 → 全量计量
  （角色/阶段/产物），预算超限升级人工（追加 / 缩减重跑 / 结束保存现状）。
- **会话与断点续跑**：文件检查点（`FileCheckpointer`）+ 会话存储（`.agent-cluster/`），
  `--resume` 从任意挂起点恢复；`/status`、`/budget`、`/skip`、`/abort` 交互命令。
- **混合需求澄清**：PM 经 `ask_user` 提问，人工自由文本回答写入 PRD 与 transcript；
  `--yes` 非交互用缺省答案并留痕。
- **自动返工 + 上限**：门 reject 回流重跑，默认 3 轮后升级人工（continue/accept/end）。
- **完整交付包**：`DELIVERY.md` 勾连需求→PRD→代码→测试→部署→手册，附 token 计量表；
