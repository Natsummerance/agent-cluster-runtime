...
const STATUS = { version: '0.6.2', projects: 1, sessions: 2, active_sessions: 1, uptime: 3600 };
...
describe('appStore', () => {
  it('refreshStatus 成功时写入状态并标记已连接', async () => {
    ...
    expect(s.status?.version).toBe('0.6.2');
  ...
  it('refreshAll 并行刷新三组数据', async () => {
    ...
    expect(useAppStore.getState().status?.version).toBe('0.6.2');
    expect(useAppStore.getState().projects).toHaveLength(1);
